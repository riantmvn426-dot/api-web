...
router.get("/api/primbon/rejeki_hoki_weton", asyncHandler(async (req, res) => {
  ...
  const result = await scrapeRejekiHokiWeton(parsedTgl.toString(), parsedBln.toString(), parsedThn.toString());
...
router.post("/api/primbon/rejeki_hoki_weton", asyncHandler(async (req, res) => {
  ...
  const result = await scrapeRejekiHokiWeton(tgl.toString(), bln.toString(), thn.toString());
...
router.metadata = {
  ...
  methods: ['GET', 'POST'],
  category: "PRIMBON",
  description: "Luck and fortune predictions based on Javanese Wetons and birth date.",
  params: [
    {
      name: "tgl",
      type: "text",
