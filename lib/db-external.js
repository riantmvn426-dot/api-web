...
var DB_BACKEND = 'mikuai-external';

console.log('');
console.log('┌──────────────────────────────────────────────────────┐');
console.log('│  🌐  MikuAI External Backend                     │');
...
// FIX: tambahkan _saveAllData sebagai no-op agar graceful shutdown di index.js
// tidak error saat DB_MODE=external (db-local punya fungsi ini, db-external tidak)
function _saveAllData(signal) {
  console.log('[db-external] _saveAllData called (' + (signal || 'unknown') + ') — no-op, data sudah persisted di remote DB.');
}
