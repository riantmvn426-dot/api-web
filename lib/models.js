...
async function getEffectiveSettings() {
  ...
  return {
    ...
    premkuEnabled : true,
    premkuMarkup  : s.premkuMarkup != null ? parseFloat(s.premkuMarkup) : 0,
    bgUrl     : s.bgUrl      || '',
...
async function saveTrx(id, d, sha) {
  ...
  // Pastikan expiresAt di result selalu angka (bukan string), cegah bug NaN pada timestamp
  if (clean.result && clean.result.expiresAt) {
    clean.result = Object.assign({}, clean.result, { expiresAt: Number(clean.result.expiresAt) || clean.result.expiresAt });
  }
...
async function updateBalance(username, delta, maxRetries) {
  var retries = maxRetries || 7; // ditingkatkan dari 3 ke 7 untuk menghindari race condition
  for (var i = 0; i < retries; i++) {
    try {
      ...
      if (!is409) throw e;
      // Backoff eksponensial: 300ms, 600ms, 900ms, ...
...
async function getPremkuOrder(id)          { return dbRead('premku-orders/' + id + '.json'); }
async function savePremkuOrder(id, d, sha) { return dbWrite('premku-orders/' + id + '.json', d, sha, 'premkuorder:' + id + ':' + (d.status||'?')); }

// ─── Premku Product Overrides ──────────────────────────────────────────────
// Stores per-product editable fields (thumbnail, desc, category, badges, etc.)
// Non-editable fields (name, price, stock, status) always come from Premku API
async function getPremkuOverrides() {
  try {
    const r = await dbRead('premku-overrides.json');
    return r.data || {};
  } catch(e) { return {}; }
}
async function savePremkuOverrides(data, sha) {
  return dbWrite('premku-overrides.json', data, sha, 'premku-overrides');
}
async function listPremkuOrders(username) {
  try {
    const files = (await listDirCached('premku-orders')).filter(function(f){ return f.name.endsWith('.json'); });
    const results = [];
    await Promise.all(files.map(async function(f) {
      try {
        const o = await getPremkuOrder(f.name.replace('.json',''));
        if (o.data && (username === null || o.data.username === username)) results.push(o.data);
      } catch(e) {}
    }));
    return results.sort(function(a,b){ return (b.createdAt||0)-(a.createdAt||0); });
...
async function saveRsServer(id, d, sha) {
  const clean = Object.assign({}, d);
  // Pastikan expiresAt selalu tersimpan sebagai angka murni — cegah bug konversi string
  if (clean.expiresAt) clean.expiresAt = Number(clean.expiresAt) || clean.expiresAt;
  return dbWrite('reseller-servers/' + id + '.json', clean, sha, 'rsserver:' + id);
}
...
module.exports = {
  ...
  getPremkuOrder, savePremkuOrder, listPremkuOrders,
  getPremkuOverrides, savePremkuOverrides,
