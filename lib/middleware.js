...
function setupCors(app) {
  ...
  app.use(cors({
    ...
    allowedHeaders: ['Content-Type', 'Authorization', 'x-admin-token', 'x-user-token', 'x-reseller-token'],
...
function setupSecurityHeaders(app) {
  app.use(function(req, res, next) {
    ...
    // CATATAN CSP: 'unsafe-inline' dipertahankan karena semua HTML menggunakan
    // inline <script> dan <style>. Untuk menghilangkannya, perlu refactor semua
    // HTML agar menggunakan external JS/CSS + nonce/hash. Ini tradeoff yang
    // disengaja — mitigasi lain (X-Frame-Options DENY, nosniff) tetap aktif.
